    ...
    <div className="min-h-screen flex flex-col items-center p-6 gap-6">
      <Card className="w-full max-w-2xl">
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            <label className="block mb-1 font-medium">Percentage of the deposit ({depositPct}%)</label>
            <Slider min={5} max={75} step={1} value={[depositPct]} onValueChange={([val]) => setDepositPct(val)} />
          </div>

          <div>
            <label className="block mb-1 font-medium">Annual interest rate ({interestRate}%)</label>
            <Slider min={2} max={6} step={0.1} value={[interestRate]} onValueChange={([val]) => setInterestRate(val)} />
          </div>

          <div>
            <label className="block mb-1 font-medium">Duration of the loan ({loanYears} years)</label>
          ...
          <div className="mt-6 p-4 bg-gray-100 rounded-lg text-sm space-y-2">
            ...
            <div>
              Term: {loanYears} years ({loanYears * 12} months)
            </div>
            <div className="text-lg font-bold">
              Monthly payment: £{monthlyPayment.toFixed(2)}
            </div>
            <div className="font-medium text-green-700">
              Ideal Household Annual Salary: £{idealSalary.toFixed(2)}
            </div>
            <div className="font-semibold">
              Total cost to be paid: £{loanAmount.toLocaleString()} (Loan) + £{totalInterest.toFixed(2)} (interest) = £{totalPayments.toFixed(1)}
            </div>
          </div>
        </CardContent>
      </Card>
